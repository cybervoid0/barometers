...
import { Image, IsAdmin, Table } from '@/components/elements'
...
function DocumentTable({ archive = [], conditions, allBarometers }: Props) {
    ...
    () => [
      ...
      display({
        ...
        cell: info => (
          <IsAdmin>
            <DocumentEdit
              key={`${info.row.original.id}-${info.row.original.updatedAt}`}
              document={info.row.original}
              conditions={conditions}
              allBarometers={allBarometers}
            />
          </IsAdmin>
        ),
      }),
    ],
    [conditions, allBarometers],
  )
